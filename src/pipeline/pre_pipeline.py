...
BASE_URL = 'https://paper-api.alpaca.markets'  # For paper trading # Ensure you have installed the Alpaca Trade API
...
class StockDataDownloader:
    ...
    def download_data(self):
        ...
        days = int(self.period[:-1])  # Extract the number of days from the string
        start_date = (datetime.now(timezone.utc) - timedelta(days=days)).strftime('%Y-%m-%dT%H:%M:%SZ')

        crypto_bars_iter = self.api.get_crypto_bars_iter(
            symbol=self.ticker,
        ...
        df2 = yf.download(tickers=str(self.ticker).replace('/','-'),period=self.period,interval=str(self.interval).replace('Min','m'))['Volume']
        self.df = df1.merge(df2, left_index=True, right_index=True, how='left')
        return self.df
...
class DataPreprocessor:
    ...
    def remove_zero_volume(self):
        self.df['Volume'].replace(0, pd.NA, inplace=True)
...
class DataProcessor:
    ...
    def create_master_signal(self):
        ...
        self.df_cleaned.loc[consecutive_zeros.shift(-4).fillna(False), 'master_signal'] = 1
...
if __name__ == "__main__":
    train_pipeline = StockDataPipeline(ticker='AVAX/USD', period='1d', interval='1Min')
